...
/**
 * Event observers used by the periods course format.
 *
...
/**
 * Event observer for format_periods.
 *
 * @package format_periods
 * @copyright 2017 Marina Glancy
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_periods_observer {
    ...
    public static function course_updated(\core\event\course_updated $event) {
        if (class_exists('format_periods', false)) {
            // If class format_periods was never loaded, this is definitely not a course in 'periods' format.
            $course = $event->get_record_snapshot('course', $event->courseid);
            format_periods::update_end_date($event->courseid);
        }
    }
    ...
    public static function course_section_created(\core\event\course_section_created $event) {
        if (class_exists('format_periods', false)) {
            // If class format_periods was never loaded, this is definitely not a course in 'periods' format.
            // Course may still be in another format but format_periods::update_end_date() will check it.
            format_periods::update_end_date($event->courseid);
        }
    }

    /**
     * Triggered via \core\event\course_section_updated event.
     *
