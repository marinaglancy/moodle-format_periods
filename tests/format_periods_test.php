...
/**
 * format_periods related unit tests
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_periods_testcase extends advanced_testcase {

    /**
     * Tests for format_periods::get_section_name method with default section names.
     */
    public function test_get_section_name() {
        ...
        $numsections = 5;
        $course = $generator->create_course(array('numsections' => $numsections, 'format' => 'periods'),
    ...
    /**
     * Tests for format_periods::get_section_name method with modified section names.
     */
    ...
    /**
     * Tests for format_periods::get_default_section_name.
     */
    public function test_get_default_section_name() {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_periods');
    ...
    public function test_update_inplace_editable() {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(array('numsections' => 5, 'format' => 'periods'),
            array('createsections' => true));
        $section = $DB->get_record('course_sections', array('course' => $course->id, 'section' => 2));

        // Call webservice without necessary permissions.
        try {
            core_external::update_inplace_editable('format_periods', 'sectionname', $section->id, 'New section name');
        ...
        $res = core_external::update_inplace_editable('format_periods', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable() {
        ...
        // Call callback format_periods_inplace_editable() directly.
        $tmpl = component_callback('format_periods', 'inplace_editable', array('sectionname', $section->id, 'Rename me again'));
    ...
    public function test_default_course_enddate() {
        ...
        $periodsformat = course_get_format($course->id);
        $this->assertEquals($enddate, $periodsformat->get_default_course_enddate($courseform->get_quick_form()));
    }
